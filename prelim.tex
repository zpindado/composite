...
\begin{assumption}[Single Group BDH-KE ($\SGBDHKE$) Assumption] Given $\gk=(p,\G_1,\G_2,\G_T,e)$, $\mathcal{G}_1$, $\mathcal{G}_2$ generators of $\G_1$, $\G_2$, respec., the SGBDH-KE Assumption holds if, for all non-uniform polynomial time adversary $\mathcal{A}$ there exists an extractor $\mathcal{X}_{\mathcal{A}}$ such that
	$$\text{Pr}\left[
	\begin{array}{c}
